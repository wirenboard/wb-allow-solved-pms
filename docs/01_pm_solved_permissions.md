...
**Хронология** (чтобы не пересматривать решение задним числом):

| Дата | Событие |
|---|---|
| 2025-02 | discourse-solved PR #334 — PM-поддержку **убрали** («don't allow **or count** solutions in PMs») |
| 2025-07-22 | `cc084bbbac` — discourse-solved **въехал в ядро** (PR #33749); история до этого в репу ядра не переехала |
| **2025-12-14** | `9158dd0` — **написан этот плагин**. На тот момент в ядре стояло безусловное `return false if topic.private_message?` — обхода не существовало |
| 2026-02-11 | `39a426505d4` — ядро добавило `allow_solved_in_groups` (PR #37635, ZogStriP), ровно под сценарий «private support workflows / internal ticket system» |

То есть плагин был написан **за 2 месяца до** появления штатной возможности и на тот момент
был единственным вариантом.

### Решения в ЛС нигде не считаются

Неочевидное и легко наступить. Ядро в #37635 открыло **только гардиан** для групповых
инбоксов, а все остальные исключения ЛС оставило нетронутыми — цитата из коммита:
*«All existing PM exclusions (reports, badges, filters, directory columns) are intentionally
left unchanged»*. В коде 2026.4.0 это живо в 8 местах:

| Файл (в `plugins/discourse-solved`) | Следствие |
|---|---|
| `app/controllers/discourse_solved/solved_topics_controller.rb:27` | список `/solved` не показывает ЛС |
| `lib/discourse_solved/register_filters.rb:11,43` | фильтры `filter:solved` проходят мимо ЛС |
| `lib/discourse_solved/queries.rb:9` | отчёты/выборки мимо ЛС |
| `lib/discourse_solved/first_accepted_post_solution_validator.rb:6` | **бейдж** за первое решение в ЛС не выдаётся |
| `plugin.rb:141,188,273` | ещё выборки, SQL и отчёты |

Все они отсекают по `archetype` (`Archetype.default` / `!= private_message`), а не по гардиану,
поэтому **плагин на них никак не влияет**: решения, отмеченные в support-инбоксе, работают
внутри треда (плашка, сортировка ответа), но **не попадают ни в один публичный счётчик**.

Для приватности это правильно — исходный PR #334 и назывался «don't allow **or count**»,
то есть проблемой было именно протекание приватной активности в публичные агрегаты.
Но практический вывод: **померить работу саппорта через solved-счётчики не получится**.
Если такая метрика понадобится — это отдельная задача (свой запрос к `discourse_solved_solved_topics`),
а не настройка.
